...
def get_max_value(df_spark, col_name):
    return df_spark.agg(max(df_spark[col_name])).collect()[0][0]

def update_date_control(last_date, minio_client, bucket_name, path): 
